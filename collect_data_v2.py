...
def create_table(conn):
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS market_data (
            ...
            UNIQUE(timestamp, symbol, interval)  -- Prevent duplicate entries
        );
        """
        cursor = conn.cursor()
...
def insert_market_data(conn, data):
    """Insert a new market data record, ignoring duplicates"""
    sql = ''' INSERT OR IGNORE INTO market_data(timestamp,open,high,low,close,volume,symbol,interval)
              VALUES(?,?,?,?,?,?,?,?) '''
    cur = conn.cursor()
    cur.execute(sql, data)
    conn.commit()
    return cur.rowcount  # Returns number of rows inserted (0 if duplicate)
...
def main():
    ...
    for kline in klines:
        ...
        inserted_count += insert_market_data(conn, data)

    print(f"Successfully processed {len(klines)} records, inserted {inserted_count} new records in {DB_NAME}")

    # Verify by reading back
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM market_data")
    count = cursor.fetchone()[0]
    print(f"Total unique records in database: {count}")
